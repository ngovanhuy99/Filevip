...
# === CẤU HÌNH TELEGRAM ===
BOT_TOKEN = "7531735205:AAG1kRT5WwFJD7KMjQ-Uzz4-OLxAaYsjaLs" #api bot
...
def send_prediction_update(session):
    ...
    message = (
        f"{EMOJI['diamond']} *SUNWIN VIP - DỰ ĐOÁN CHUẨN 87%* {EMOJI['diamond']}\n"
        f"══════════════════════════\n"
...
def background_task():
    ws_url = "ws://167.172.85.59:8000/game_sunwin/ws?id=duyduy&key=khanhnnasga1" 
    reconnect_delay = 1
    while True:
        try:
            ...
            reconnect_delay = min(reconnect_delay * 0.1, 0.5)
                ...
                if "callback_query" in update:
                    ...
                    if data == "help_activate":
                        help_message = (
                            ...
                            f"══════════════════════════\n"
                            f"{EMOJI['team']} Liên hệ admin: @tungtung2121"
                        )
                    ...
                    if text:
                        if text.startswith("/start"):
                            welcome_message = (
                                ...
                                f"- Nhập `/ <key_của_bạn>` để kích hoạt bot\n"
                            ...
                            buttons = [
                                ...
                                [{"text": f"{EMOJI['money_bag']} Liên hệ mua key", "url": "https://t.me/tungtung2121"}]
                            ]
...
def main():
    ...
    if c.fetchone()[0] == 0:
        ...
        c.execute("INSERT INTO admins (chat_id) VALUES (?)", ("7071414779",))
        conn.commit()
    conn.close()
...
# === PATTERN NÂNG CAO CÓ GIẢI THÍCH (GỘP THÊM) ===
ADVANCED_PATTERN = {
...
def analyze_advanced_pattern(history):
    pattern_str = ",".join([s["result"][0].upper() for s in history[:7]])
    cau_text = pattern_str.replace(",", "")
    prediction = None
    confidence = 0
    details = []

    if pattern_str in ADVANCED_PATTERN:
        data = ADVANCED_PATTERN[pattern_str]
        prediction = "Tài" if data["prediction"] == "T" else "Xỉu"
        confidence = data["probability"]
        if "description" in data:
            details.append(data["description"])
        else:
            details.append(f"Pattern nâng cao ({pattern_str})")

    if is_cau_dep(cau_text):
        details.append("Cầu đẹp ✅")
    elif is_cau_xau(cau_text):
        details.append("Cầu xấu ⚠️")

    return prediction, confidence, " | ".join(details) if details else None